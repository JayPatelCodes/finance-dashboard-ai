...
async def forecast(current_user: dict = Depends(get_current_user)):
    ...
    df = DataFrame(data)
    if df.empty:
        return {"points": [], "summary": "No data to forecast. Upload a CSV first."}
    ...
    points = [
        {"date": d.strftime("%Y-%m-%d"), "predicted": float(p)}
        for d, p in zip(future_dates, preds)
    ]
    summary = f"Predicted average daily spend for the next 30 days: ${abs(float(np.mean(preds))):.2f}."
